...
class BorutaShap:
    ...
    def Check_if_chose_train_or_test_and_train_model(self):
        ...
        if self.train_or_test.lower() == 'test':
            ...
            # Added line for using test set in deriving shap values (Yixian)
    ...
    def explain(self):
        ...
        if self.sample:
            ...
            if self.classification:
                # for some reason shap returns values wraped in a list of length 1
                # Modify line 737 to use X_boruta_test instead of X_boruta if using train test split
                self.shap_values = np.array(explainer.shap_values(self.X_boruta_test))
                if isinstance(self.shap_values, list):
                ...
                # Modify line 755 to use X_boruta_test instead of X_boruta if using train test split
